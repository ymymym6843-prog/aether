export const DomRenderer = {
    ...
    renderWeatherMap(current) {
        ...
        if (cloudLayer) {
            ...
            for (let i = 0; i < cloudCount; i++) {
                ...
                cloudGroup.style.left = `${10 + Math.random() * 60}%`;
                cloudGroup.style.top = `${10 + Math.random() * 50}%`;
                cloudGroup.style.animationDelay = `${Math.random() * 5}s`;
                cloudGroup.style.animationDuration = `${16 + Math.random() * 6}s`; // Varied speed

                // Create 3-4 bubbles per cloud for organic shape
                const bubbleCount = 3 + Math.floor(Math.random() * 2);
                for (let j = 0; j < bubbleCount; j++) {
                    const bubble = document.createElement('span');
                    bubble.className = 'cloud-bubble';
                    bubble.style.left = `${j * 25 + (Math.random() * 15) - 7.5}px`;
                    bubble.style.top = `${(Math.random() * 20) - 10}px`;
                    bubble.style.width = `${60 + Math.random() * 60}px`;
                    bubble.style.height = `${40 + Math.random() * 40}px`;
        ...
        if (precipLayer) {
            ...
            if (showPrecip) {
                const dropCount = Math.min(60, Math.max(20, Math.round((precipChance || 30) / 1.5))); // More drops

                // Dynamic speed based on precipitation intensity
                const speedMultiplier = Math.max(0.5, (precipChance || 30) / 50);
                ...
                for (let i = 0; i < dropCount; i++) {
                    ...
                    // Faster rain = heavier rain
                    const baseDuration = precipType === 'snow' ? 3.5 : 0.8;
                    drop.style.animationDuration = `${(baseDuration / speedMultiplier) + Math.random() * 0.3}s`;
    ...
    renderTrendChart(daily) {
        requestAnimationFrame(() => {
            ...
            // Ensure canvas takes full width of container (minus padding)
            canvas.style.width = '100%';
            const rect = canvas.getBoundingClientRect();

            if (rect.width === 0) {
                // Retry once if width is 0 (e.g. tab transition)
                requestAnimationFrame(() => this.renderTrendChart(daily));
                return;
            }
            ...
            // Gradient Line
            const grad = ctx.createLinearGradient(paddingX, 0, w - paddingX, 0);
            grad.addColorStop(0, '#00F2FF');
            grad.addColorStop(1, '#BC13FE');

            // Draw High Temp Line
            ctx.beginPath();
            const step = drawWidth / (daily.length - 1);
